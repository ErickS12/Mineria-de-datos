...
def abrir_ventana():
    ventana = tk.Toplevel()
    ventana.title("Simulador Normal con media y varianza")
    ...
    entry_mues = tk.Entry(input_frame)
    entry_mues.insert(0, "1000")
    entry_mues.pack(side=tk.LEFT, padx=5)

    tk.Label(input_frame, text="Media:").pack(side=tk.LEFT, padx=5)
    entry_med= tk.Entry(input_frame)
    entry_med.insert(0, "1000")
    entry_med.pack(side=tk.LEFT, padx=5)

    tk.Label(input_frame, text="Varianza:").pack(side=tk.LEFT, padx=5)
    entry_var= tk.Entry(input_frame)
    entry_var.insert(0, "1000")
    entry_var.pack(side=tk.LEFT, padx=5)
    ...
    def simu():
        try:
            repe = int(entry_mues.get())
            mu = float(entry_med.get())
            sigma_2 = float(entry_var.get())
            sigma_1 = math.sqrt(sigma_2)
        ...
        N, bins, patches = ax.hist(Y, bins=30, color='skyblue', edgecolor='black')
        ...
        ax.set_title('Histograma de muestreo de una normal con media y varianza')
        ax.set_xlabel('Valor')
